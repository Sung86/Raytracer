...
// ThreadData structure to hold all the parameters necessary for running the thread function
struct ThreadData
{
...
int render(Scene* scene, const int width, const int height, const int aaLevel, bool testMode, int threads, int threadId, int blockSize, unsigned int *blockCount, bool colourise)
{
	...
	//  width of each block eg. 1024/8 is 18
	unsigned int widthInBlocks = width / blockSize;
	//  height of each block eg. 1024/8 is 18
	unsigned int heightInBlocks = height / blockSize;

	unsigned int blockNum;

	// loop through all the pixels.
	while ((blockNum = InterlockedIncrement(blockCount)) < widthInBlocks * heightInBlocks)
	{	
		//calculate the starting index of blocks
		int blockx = blockNum % widthInBlocks;
		int blocky = blockNum / widthInBlocks;

		// caluculating the last index of blocks 
		// as loop goes from -width/2 to width/2, the first block start from zero must be shift to -width/2
		int lastx = ((blockx + 1) * blockSize) - (width / 2);
		// as loop goes from -height/2 to height/2, the first block start from zero must be shift to -height/2
		int lasty = ((blocky + 1) * blockSize) - (height / 2);

		// finding if the block is the last one in height, then last should be the end of height
		// as some times height is not completely divided by blocks . some some lines are left unprocessed
		if (blockNum / widthInBlocks == (heightInBlocks)-1) 
		{
			lasty = height / 2;
		}
		// finding if block is last one in width , the last would be end of width
		if ((blockNum % widthInBlocks) == widthInBlocks - 1) 
		{
			lastx = width / 2;
		}

		// loop in side each blocks
		for (int y = (blocky * blockSize) - (height / 2); y < lasty; ++y) 
		{
			for (int x = (blockx * blockSize) - (width / 2); x < lastx; ++x)
			{
				...
				else
				{
					...
					out[(y + (height / 2)) * width + (x + (width / 2))] = Colour(1.0 / colourCode, 1.0 / colourCode, 1.0 / colourCode).convertToPixel();
				}
			}
		}
	}
...
DWORD __stdcall ThreadStart(LPVOID threadData)
{
	...
	samplesRendered = 0;
	samplesRendered += render(data->scene, data->width, data->height, data->samples, data->testmode, data->threads, data->threadID, data->blockSize, data->blockCount, data->colourise);
	ExitThread(NULL);
}

//generates threads for processing the image
static void generate(unsigned int threads, Scene*  scene, int width, int height, int samples, int testMode, int blockSize, bool colourise)
{
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < times; i++)
	{
		...
		generate(threads, &scene, width, height, samples, testMode, blockSize, colourise);	// raytrace scene
