...
int render(Scene* scene, const int width, const int height, const int aaLevel, bool testMode, int threads, int threadId, unsigned int *lineCount, bool colourise)
{
	...
	int y;
	while ((y = InterlockedIncrement(lineCount) + (-height / 2)) < height / 2)
	{
		for (int x = -width / 2; x < width / 2; ++x)
		{
			// as this number is used in division. and thread can have 0 id. to avoid error starting from 1
...
DWORD __stdcall threadStart(LPVOID threadData)
{
	...
	samplesRendered += render(data->scene, data->width, data->height, data->samples, data->testmode, data->threads, data->threadID, data->lineCount, data->colourise);
	ExitThread(NULL);
}

//generates threads for processing the image
static void generate(unsigned int threads, Scene*  scene, int width, int height, int samples, bool testMode, bool colourise)
{
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < times; i++)
	{
		...
		generate(threads, &scene, width, height, samples, testMode, colourise);	    // call function to generate threads for retrace 
